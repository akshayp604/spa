...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	while (slotTime < this.maxTime) {
            ...
            dateis = new Date(slotDate);

            axisHtml =
                '<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
                    (isLabeled ?
                        '<span class="slot_main">' + // for matchCellWidths
                            moment(slotDate).format('hh:mm') +
                        '</span><span class="slot_main_mom">'+moment(slotDate).format('A')+'</span>' :
                        ''
                        ) +
                '</td>';
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		classes.unshift('fc-time-grid-event', 'fc-v-event' , 'style_popup');
